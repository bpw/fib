...
public class BootImage extends BootImageWriterMessages
  ...
  private void markReferenceMap(Address address) {
    ...
    if (referenceIndex < 0 || referenceIndex > referenceMap.length) {
      int objRef = address.minus(MiscHeader.FIB_OFFSET).toInt() >> LOG_BYTES_IN_ADDRESS;
      System.out.println("Uh oh.  referenceIndex = " + referenceIndex + "  referenceMap.length = " + referenceMap.length + "  objRef would be = " + objRef);
    }
