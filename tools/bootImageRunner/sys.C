...
extern "C" void sysMonitorSignal(Word);
...
extern "C" void
sysMonitorSignal(Word _monitor)
{
#ifdef RVM_FOR_HARMONY
    hythread_monitor_notify((hythread_monitor_t)_monitor);
#else
    vmmonitor_t *monitor = (vmmonitor_t*)_monitor;
...
sysMalloc(int length)
{
    ...
    if (inRVMAddressSpace((Address)result)) {
      ...
      if ((int)result < 0x40000000 || (int)result >= 0x50000000) {
