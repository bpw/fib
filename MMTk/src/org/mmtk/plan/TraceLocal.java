...
public abstract class TraceLocal extends TransitiveClosure implements Constants {
  ...
  public final void processRootEdge(Address slot, boolean untraced) {
    ...
    // FIB: note the only Plan that actually does more than load is Poisoned
    // and it also uses the one bit, so we're not compatible...
    // FIB: if we catch an epoch, skip it.
    if (VM.scanning.isNonRef(object)) return;
    // end FIB
  ...
  public void processRoots() {
    ...
    while (!rootLocations.isEmpty()) {
      // FIB: TODO not sure if epoch checking is needed here.
