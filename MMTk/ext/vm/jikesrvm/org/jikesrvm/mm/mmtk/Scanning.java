...
public final class Scanning extends org.mmtk.vm.Scanning implements Constants {
  ...
  public void scanObject(TransitiveClosure trace, ObjectReference object) {
    // FIB:
    if (Dr.SCAN) ObjectShadow.scan(trace, object);
  ...
  public void computeGlobalRoots(TraceLocal trace) {
    ...
    for(int i=start; i < end; i++) {
      // FIB: don't think we can get any epochs here.
  ...
  @Uninterruptible
  @Override
  public boolean isNonRef(ObjectReference objectRef) {
    return Dr.CHECKS && !Epoch.isRef(objectRef.toAddress().toWord());
  }
}
