...
@Uninterruptible public class Memory extends org.mmtk.vm.Memory
  ...
  public final void globalPrepareVMSpace() {
    bootSpace.prepare();
    // FIB:
    if (Dr.ON) DrRuntime.prepareGlobal();
  }

  // FIB: added argument
  @Override
  public final void collectorPrepareVMSpace(MutatorContext mc) {
    // FIB:
    if (Dr.ON) DrRuntime.preparePerThread(mc);
  }

  // FIB: added argument
  @Override
  public final void collectorReleaseVMSpace(MutatorContext mc) {
    // FIB:
    if (Dr.ON) DrRuntime.releasePerThread(mc);
  }
  ...
  public final void globalReleaseVMSpace() {
    bootSpace.release();
    // FIB:
    if (Dr.ON) DrRuntime.releaseGlobal();
  }
