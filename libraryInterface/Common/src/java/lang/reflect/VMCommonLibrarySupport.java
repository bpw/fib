...
final class VMCommonLibrarySupport {
  ...
  throws InstantiationException, IllegalAccessException, IllegalArgumentException, InvocationTargetException {
    ...
    // Escape: Mark the object escaped, if allocated in VM context.
    if (Esc.getConfig().escapeVMContextEscape() && VM.runningVM && Magic.getStaticContext() == Context.VM_CONTEXT) {
      EscapeState.markObjectEscaped(obj);
    }
  ...
  throws IllegalAccessException, IllegalArgumentException     {
    ...
    if (field.isReferenceType()) {
      if (value != null) {
        ...
        // Escape: Add write barrier if this method is in application context. This barrier is not needed if instrumenting VM.
        if (Esc.getConfig().escapeInsertBarriers() && !Esc.getConfig().escapeInstrumentVM() && Magic.getStaticContext() == Context.APP_CONTEXT) {
          if (field.isStatic()) {
            EscapeState.writeStatic(ObjectReference.fromObject(value).toAddress().toWord(),
                field.getOffset(),
                EscInstrDecisions.passFieldInfo() ? (Esc.getConfig().escapeUseFieldOffset() ? field.getOffset().toInt() : field.getId()) : 0,
                0);
          } else {
            EscapeState.writeObject(ObjectReference.fromObject(value).toAddress().toWord(),
                ObjectReference.fromObject(object).toAddress(),
                field.getOffset(),
                EscInstrDecisions.passFieldInfo() ? (Esc.getConfig().escapeUseFieldOffset() ? field.getOffset().toInt() : field.getId()) : 0,
                0);
          }
