...
public class GenerateMagic implements TIBLayoutConstants  {
  ...
  static boolean generateMagic(BC2IR bc2ir, GenerationContext gc, MethodReference meth)
    ...
    // Octet: magic methods for introspection
    } else if (methodName == MagicNames.getSiteID) {
      int siteID = Octet.getClientAnalysis().needsSites() ? Site.getSite(gc.inlineSequence, bc2ir.getInstrIndex(), true) : 0;
      bc2ir.push(IRTools.IC(siteID));
    } else if (methodName == MagicNames.getStaticContext) {
      bc2ir.push(IRTools.IC(gc.method.getStaticContext()));
    } else {
