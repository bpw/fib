...
public abstract class BaselineCompilerImpl extends BaselineCompiler implements BaselineConstants, SizeConstants {
  ...
  static {
    ...
    // Octet: added extra param (biStart)
    Object x = BaselineMagic.generateMagic(null, null, null, Offset.zero(), 0);
  }
  ...
  // FIB: instrument?
  final boolean fib = Dr.ON && Octet.shouldInstrumentMethod(this.method);
  final boolean fibLocks = Dr.LOCKS && fib;
  
  /** Escape: baselin instrumentation helper */
  final EscapeBaselineInstr escapeBaselineInstr = new EscapeBaselineInstr();
  ...
  protected final void emit_aastore() {
    ...
    // Esc: insert barrier
    escapeBaselineInstr.insertArrayWriteBarrier(method, biStart, TWO_SLOTS, TypeReference.ObjectReference, asm);
  ...
  protected final void emit_unresolved_getstatic(FieldReference fieldRef) {
    ...
    if (NEEDS_OBJECT_GETSTATIC_BARRIER && !fieldRef.getFieldContentsType().isPrimitiveType()) {
      ...
      // FIB: potentially insert post-barrier
      octetBaselineInstr.insertStaticPostBarrierUnresolved(method, biStart, true, fieldRef, asm);
      return;
    }
  ...
  protected final void emit_resolved_getstatic(FieldReference fieldRef) {
    ...
    if (NEEDS_OBJECT_GETSTATIC_BARRIER && !fieldRef.getFieldContentsType().isPrimitiveType() && !field.isUntraced()) {
      ...
      // FIB: potentially insert post-barrier
      octetBaselineInstr.insertStaticPostBarrierResolved(method, biStart, true, field, asm);
      return;
    }
  ...
  protected final void emit_unresolved_putstatic(FieldReference fieldRef) {
    ...
    // Esc: insert barrier
    escapeBaselineInstr.insertWriteBarrierStaticUnresolved(method, biStart, T0, fieldRef, asm);
    ...
    // FIB: potentially insert post-barrier
    octetBaselineInstr.insertStaticPostBarrierUnresolved(method, biStart, false, fieldRef, asm);
  ...
  protected final void emit_resolved_putstatic(FieldReference fieldRef) {
    ...
    // Esc: insert barrier
    escapeBaselineInstr.insertWriteBarrierStaticResolved(method, biStart, field, asm);
    ...
    // FIB: potentially insert post-barrier
    octetBaselineInstr.insertStaticPostBarrierResolved(method, biStart, false, field, asm);
  ...
  protected final void emit_unresolved_getfield(FieldReference fieldRef) {
    ...
    // FIB: potentially insert post-barrier
    octetBaselineInstr.insertFieldPostBarrierUnresolved(method, biStart, true, fieldRef, asm);
  ...
  protected final void emit_resolved_getfield(FieldReference fieldRef) {
    ...
    // FIB: potentially insert post-barrier
    octetBaselineInstr.insertFieldPostBarrierResolved(method, biStart, true, field, asm);
  ...
  protected final void emit_unresolved_putfield(FieldReference fieldRef) {
    ...
    if (fieldType.isReferenceType()) {
      // Esc: insert barrier
      escapeBaselineInstr.insertWriteBarrierFieldUnresolved(method, biStart, ONE_SLOT, T0, fieldRef, asm);
    ...
    // FIB: potentially insert post-barrier
    octetBaselineInstr.insertFieldPostBarrierUnresolved(method, biStart, false, fieldRef, asm);
  ...
  protected final void emit_resolved_putfield(FieldReference fieldRef) {
    ...
    if (field.isReferenceType()) {
      // Escape: insert barrier
      escapeBaselineInstr.insertWriteBarrierFieldResolved(method, biStart, ONE_SLOT, field, asm);
    ...
    // FIB: potentially insert post-barrier
    octetBaselineInstr.insertFieldPostBarrierResolved(method, biStart, false, field, asm);
  ...
  protected final void emit_unresolved_invokevirtual(MethodReference methodRef) {
    emitDynamicLinkingSequence(asm, T0, methodRef, true, method.getStaticContext());            // T0 has offset of method
  ...
  protected final void emit_resolved_invokevirtual(MethodReference methodRef) {
    int methodRefparameterWords = methodRef.getParameterWords() + 1; // +1 for "this" parameter
    Offset methodRefOffset = methodRef.peekResolvedMethod(method.getStaticContext()).getOffset();
    Offset objectOffset =
      Offset.fromIntZeroExtend(methodRefparameterWords << LG_WORDSIZE).minus(WORDSIZE); // object offset into stack
    stackMoveHelper(T1, objectOffset);                               // T1 has "this" parameter
    baselineEmitLoadTIB(asm, S0, T1);                                // S0 has TIB
  ...
  protected final void emit_resolved_invokestatic(MethodReference methodRef) {
    Offset methodOffset = methodRef.peekResolvedMethod(method.getStaticContext()).getOffset();
    genParameterRegisterLoad(methodRef, false);
  ...
  protected final void emit_invokeinterface(MethodReference methodRef) {
    ...
    if (VM.BuildForIMTInterfaceInvocation) {
      InterfaceMethodSignature sig = InterfaceMethodSignature.findOrCreate(methodRef, method.getStaticContext());
  ...
  protected final void emit_resolved_new(RVMClass typeRef) {
    ...
    // Escape: Mark the object escaped if in VM context
    escapeBaselineInstr.markEscaped(method, biStart, typeRef.getTypeRef(), asm);
  }
  ...
  protected final void emit_unresolved_new(TypeReference typeRef) {
    ...
    // Escape: Mark the object escaped if in VM context
    escapeBaselineInstr.markEscaped(method, biStart, typeRef, asm);
  }
  ...
  protected final void emit_resolved_newarray(RVMArray array) {
    ...
    // Escape: Mark the object escaped if in VM context
    escapeBaselineInstr.markEscaped(method, biStart, array.getTypeRef(), asm);
  }
  ...
  protected final void emit_unresolved_newarray(TypeReference tRef) {
    ...
    // Escape: Mark the object escaped if in VM context
    escapeBaselineInstr.markEscaped(method, biStart, tRef, asm);
  }
  ...
  protected final void emit_monitorenter() {
    ...
    asm.emitCALL_Abs(Magic.getTocPointer().plus((fibLocks ? Entrypoints.drLockMethod : Entrypoints.lockMethod).getOffset()));
  }
  ...
  protected final void emit_monitorexit() {
    genParameterRegisterLoad(asm, 1);          // pass 1 parameter word
    // FIB: choice of method
    asm.emitCALL_Abs(Magic.getTocPointer().plus((fibLocks ? Entrypoints.drUnlockMethod : Entrypoints.unlockMethod).getOffset()));
  }
  ...
  private boolean genMagic(MethodReference m) {
    // Octet: Pass through the bytecode index, too
    if (BaselineMagic.generateMagic(asm, m, method, fp2spOffset(NO_SLOT), biStart)) {
      return true;
    } else if (m.isSysCall()) {
