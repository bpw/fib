...
final class BaselineMagic {
  ...
  static boolean generateMagic(Assembler asm, MethodReference m, RVMMethod cm, Offset sd, int biStart) {
    MagicGenerator g = generators.get(m);
    if (g != null) {
      g.generateMagic(asm, m, cm, sd, biStart);
      return true;
    } else {
      return false;
    }
  ...
  private abstract static class MagicGenerator {
    ...
    // Octet: Added ability to send bytecodex index
    void generateMagic(Assembler asm, MethodReference m, RVMMethod cm, Offset sd, int biStart) {
      generateMagic(asm, m, cm, sd);
    }
  }
  ...
  // Octet: Magic to do compile-time introspection
  // Octet: TODO: these changes are partially related to static cloning
  
  private static final class Magic_getSiteID extends MagicGenerator {
    @Override
    void generateMagic(Assembler asm, MethodReference m, RVMMethod cm, Offset sd) {
    ...
    @Override
    void generateMagic(Assembler asm, MethodReference m, RVMMethod cm, Offset sd, int biStart) {
      int siteID = Octet.getClientAnalysis().needsSites() ? Site.getSite(cm, biStart, null, true) : 0;
      asm.emitPUSH_Imm(siteID);
    }
  }
  static {
    MagicGenerator g = new Magic_getSiteID();
    generators.put(getMethodReference(Magic.class, MagicNames.getSiteID, int.class), g);
  }
  
  private static final class Magic_getStaticContext extends MagicGenerator {
    @Override
    void generateMagic(Assembler asm, MethodReference m, RVMMethod cm, Offset sd) {
      int context = cm.getStaticContext();
      asm.emitPUSH_Imm(context);
    }
  }
  static {
    MagicGenerator g = new Magic_getStaticContext();
    generators.put(getMethodReference(Magic.class, MagicNames.getStaticContext, int.class), g);
  }
}
