...
public final class MiscHeader implements Constants, MiscHeaderConstants {
  ...
  public static final Offset OCTET_OFFSET = MISC_HEADER_START.plus(GC_TRACING_HEADER_BYTES);
  
  /** FIB: header word offset */
  public static final Offset FIB_OFFSET = OCTET_OFFSET.plus(OCTET_HEADER_BYTES);
  
  /** FIB: second header word offset */
  public static final Offset FIB_EXTRA_OFFSET = FIB_OFFSET.plus(BYTES_IN_WORD);
  
  /** Escape: extra header word's offset, immediately follows FIB's header word(s) */
  public static final Offset ESCAPE_OFFSET = FIB_OFFSET.plus(FIB_HEADER_BYTES);
   ...
   * Maybe 1 for escape.
   * Definitely one for tagging array shadows.
   */
  static final int REQUESTED_BITS = Esc.getConfig().escapeUseOneBitInGCHeader() ? 2 : 1;
  ...
  public static void initializeHeader(Object obj, TIB tib, int size, boolean isScalar) {
    ...
    // Escape: initialize the metadata
    if (Esc.getConfig().escapeInstrumentAllocation()) {
      Word metadata = EscapeState.getInitialMetadata();
      EscapeState.checkMetadata(metadata, obj, 0, true);
      ObjectReference.fromObject(obj).toAddress().store(metadata, MiscHeader.ESCAPE_OFFSET);
      EscapeStats.Alloc_Runtime.inc();
      EscapeStats.Alloc_Runtime_inHarness.inc();
      if (Esc.getConfig().escapeStats()) {
        if (Context.isVMPrefix(tib.getType().getTypeRef())) {
          EscapeStats.Alloc_Runtime_VM.inc();
        } else {
          EscapeStats.Alloc_Runtime_NonVM.inc();
        }
      }
    }
    if (Esc.getConfig().escapeUseOneBitInGCHeader()) {
      // The default value should be 0, so the object is not escaped.
      EscapeStats.Alloc_Runtime.inc();
      EscapeStats.Alloc_Runtime_inHarness.inc();
      if (Esc.getConfig().escapeStats()) {
        if (Context.isVMPrefix(tib.getType().getTypeRef())) {
    ...
    // FIB: initialize the metadata
    if (Dr.HEADER) ObjectShadow.initializeHeader(obj, tib, size, isScalar);
  ...
  public static void initializeHeader(BootImageInterface bootImage, Address ref, TIB tib, int size,
    ...
    // Escape: initialize boot image objects
    if (Esc.getConfig().escapeInstrumentAllocation()) {
      Word metadata = EscapeState.getInitialMetadataInBootImageBuild();
      bootImage.setAddressWord(ref.plus(ESCAPE_OFFSET), metadata, false, false);
    }
    EscapeStats.Alloc_BootImage.inc();
    
    // FIB: initialize the metadata
    if (Dr.HEADER) ObjectShadow.initializeHeader(bootImage, ref, tib, size, isScalar);
