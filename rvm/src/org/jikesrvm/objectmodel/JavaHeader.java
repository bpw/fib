...
public class JavaHeader implements JavaHeaderConstants {
  ...
  // Escape: It seems that in baseline compiler we cannot directly use EscapeState.stateBitOffset. Add the constant here.
  public static final Offset ESCAPE_BIT_OFFSET = STATUS_OFFSET;
  public static final Offset FIB_BITS_OFFSET = AVAILABLE_BITS_OFFSET;
  /*
  ...
  public static void setTIB(Object ref, TIB tib) {
    ...
    // Man: Not sure if we really need to insert this barrier. Alternative is to mark all TIB objects as ESCAPED.
    if (Esc.getConfig().escapeInsertBarriers() && Esc.getConfig().escapeInstrumentVM()) {
      EscapeState.writeObject(ObjectReference.fromObject(tib).toAddress().toWord(),
          ObjectReference.fromObject(ref).toAddress(),
          TIB_OFFSET,
          0,
          0);
    }
  ...
  protected static int installHashCode(Object o) {
    ...
    // Escape: Make sure this method never runs.
