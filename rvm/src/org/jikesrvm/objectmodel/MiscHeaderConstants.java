...
public interface MiscHeaderConstants extends Constants {
  ...
  /** FIB: header bytes */
  int FIB_HEADER_BYTES = Dr.HEADER_WORDS * BYTES_IN_WORD;
  
  /** Escape: number of extra header bytes; also used below to calculate NUM_BYTES_HEADER */
  int ESCAPE_HEADER_BYTES = Esc.getConfig().escapeAddHeaderWord() ? BYTES_IN_WORD : 0;

  /**
   * How many bytes are used by all misc header fields?
   */
  int NUM_BYTES_HEADER = GC_TRACING_HEADER_BYTES + OCTET_HEADER_BYTES + FIB_HEADER_BYTES + ESCAPE_HEADER_BYTES; // + YYY_HEADER_BYTES;
}
