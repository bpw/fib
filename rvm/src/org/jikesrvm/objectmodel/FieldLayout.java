...
public abstract class FieldLayout implements SizeConstants {
  ...
  */
  ...
  // FIB
  private Offset offsetToObjectOffset(RVMClass klass, int offset) {
    Offset fieldOffset;
    if (offset >= 0) {
      fieldOffset =
  ...
  protected void setOffset(RVMClass klass, RVMField field, int offset) {
    ...
    // FIB: factored out this part
    Offset fieldOffset = offsetToObjectOffset(klass, offset);
  ...
  protected void layoutField(FieldLayoutContext layout, RVMClass klass, RVMField field, int fieldSize) {
    ...
    // FIB: add space for history
    if (FieldTreatment.check(field)) {
      field.setDrOffset(offsetToObjectOffset(klass, layout.nextOffset(AccessHistory.BYTES_IN_HISTORY, false)));
      if (DEBUG) {
        VM.sysWriteln("  DR offset (instance, history) = ", field.getDrOffset().toInt());
      }
    } else if (FieldTreatment.vol(field)) {
      field.setDrOffset(offsetToObjectOffset(klass, layout.nextOffset(BYTES_IN_ADDRESS, true)));
      if (DEBUG) {
        VM.sysWriteln("  DR offset (instance, vol. VC) = ", field.getDrOffset().toInt());
      }
    }
  }
}
