...
public abstract class RVMMethod extends RVMMember implements BytecodeConstants {
  ...
  /** Because of inheritance, a method with just one static context may multiple resolved methods (one for each context). */
    ...
    if (isCompiled()) {
      ...
      if (!isStatic() && !isObjectInitializer() && !isPrivate()) {
        ...
        if (declaringClass.isJavaLangObjectType() ||
            declaringClass.getSuperClass().findVirtualMethod(getName(), getDescriptor(), getResolvedContext()) == null) {
