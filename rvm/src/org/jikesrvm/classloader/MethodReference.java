...
public final class MethodReference extends MemberReference {
  ...
  private RVMMethod resolvedMember(int context) {
    if (context == Context.VM_CONTEXT) {
      ...
      return resolvedMemberAPP;
    }
  }

  private RVMMethod resolvedMemberVM;
  private RVMMethod resolvedMemberAPP;

  /** This method is weird because there's a possible race -- but these variables should be set by the time it's called. */
  @Uninterruptible
  boolean hasMultipleResolvedContexts() {
    if (VM.VerifyAssertions) { VM._assert(resolvedMemberVM != null && resolvedMemberAPP != null); }
    return resolvedMemberVM != null && resolvedMemberAPP != null && resolvedMemberVM != resolvedMemberAPP;
  }
  ...
  */
  ...
  void setResolvedMember(RVMMethod it, boolean checkAssertion) {
    //if (VM.VerifyAssertions && checkAssertion) VM._assert(resolvedMember(resolvedContext) == null || resolvedMember(resolvedContext) == it);
    if (!Context.hasMultipleStaticContexts(this) && it.getStaticContext() == it.getResolvedContext()) {
      if (resolvedMemberVM == null) {
        resolvedMemberVM = it;
      }
      if (resolvedMemberAPP == null) {
        resolvedMemberAPP = it;
      }
    } else {
      if (it.getResolvedContext() == Context.VM_CONTEXT) {
        resolvedMemberVM = it;
      } else {
        if (VM.VerifyAssertions) { VM._assert(it.getResolvedContext() == Context.APP_CONTEXT); }
        resolvedMemberAPP = it;
      }
    }
  }
