...
public final class RVMClass extends RVMType implements Constants, ClassLoaderConstants {
  ...
  public RVMMethod findDeclaredMethod(Atom methodName, Atom methodDescriptor, int context) {
    ...
    for (RVMMethod method : declaredMethods) {
      if (method.getName() == methodName && method.getDescriptor() == methodDescriptor) {
        foundSomething = true;
        if (Context.isMatch(method, context)) {
          return method;
        }
      }
    }
  ...
  public RVMMethod findMainMethod() {
    ...
    RVMMethod mainMethod = this.findDeclaredMethod(mainName, mainDescriptor, Context.APP_CONTEXT);
  ...
  public RVMMethod findStaticMethod(Atom memberName, Atom memberDescriptor, int context) {
    ...
    for (RVMMethod method : getStaticMethods()) {
      if (method.getName() == memberName && method.getDescriptor() == memberDescriptor &&
          Context.isMatch(method, context)) {
        return method;
      }
    }
    return null;
  ...
  public synchronized void resolve() {
    ...
    //
    {
      ...
      for (int j = 0, m = virtualMethods.size(); j < m; j++) {
        ...
        if (overridingMethod != null && !hasContextMatch) {
          RVMMethod clonedMethod = overridingMethod.cloneMethod(Context.getOtherContext(overridingMethod.getResolvedContext()));
          ...
          for (RVMMethod iMeth : iMeths) {
            ...
            for (RVMMethod declaredMethod : declaredMethods) {
              if (declaredMethod.getName() == iMeth.getName() && declaredMethod.getDescriptor() == iMeth.getDescriptor()) {
                ...
                if (Context.isMatch(iMeth, declaredMethod.getResolvedContext())) {
                  overridingMethod = declaredMethod;
                  break;
                }
              }
            ...
            if (partialMatch != null && overridingMethod == null) {
              RVMMethod newDeclaredMethod = partialMatch.cloneMethod(Context.getOtherContext(partialMatch.getResolvedContext()));
          ...
          for (RVMMethod iMeth : iMeths) {
            ...
            for (int k = 0; k < virtualMethods.size(); k++) {
              ...
              if (vMeth.getName() == iName && vMeth.getDescriptor() == iDesc) {
                ...
                if (Context.isMatch(iMeth, vMeth.getResolvedContext())) continue outer;
              }
            }
    ...
    for (RVMField field : staticFields) {
      ...
      if (FieldTreatment.check(field)) {
        // slots, non-contiguous with data
        field.setDrJTOCOffset(Statics.allocateDrAccessHistory());
      } else if (FieldTreatment.vol(field)) {
        // 1 slot, non-contiguous with data.
        field.setDrJTOCOffset(Statics.allocateReferenceSlot(false));
      }
    ...
    for (RVMField field : instanceFields) {
      ...
      // FIB:
      switch (FieldTreatment.treatment(field)) {
      case CHECK:
        drCheckedFieldCount++;
        break;
      case VOLATILE:
    ...
    if (typeRef.isRuntimeTable()) {
      ...
      // FIB:
      drAccessHistoryOffsets = null;
      // end FIB
    } else {
      referenceOffsets = MemoryManager.newNonMovingIntArray(referenceFieldCount);
      // FIB:
      drAccessHistoryOffsets = 
          drCheckedFieldCount == 0 ? null : MemoryManager.newNonMovingIntArray(drCheckedFieldCount);
      int k = 0;
      // end FIB
      int j = 0;
      for (RVMField field : instanceFields) {
        ...
        // FIB:
        if (FieldTreatment.check(field)) {
          drAccessHistoryOffsets[k++] = field.getDrOffset().toInt();
        } else if (FieldTreatment.vol(field)) {
          referenceOffsets[j++] = field.getDrOffset().toInt();
        }
        // end FIB
      }
    }
  ...
  public synchronized void initialize() throws ExceptionInInitializerError {
    ...
    if (classInitializerMethod != null) {
      ...
      try {
        ...
        eieio.initCause(t);
        state = CLASS_INITIALIZER_FAILED;
        if (VM.verboseClassLoading || Dr.ON) {
  ...
  // DR
  @Uninterruptible
  public final boolean hasStaticFields() {
    return staticFields.length != 0;
  }
  private WordArray drClassInitVC;
//  public boolean[] fibObservedBy;
//  @Inline
//  @Uninterruptible
//  public final boolean fibThreadHasObservedInit() {
//    if (Dr.STATS) DrStats.staticFinalReads.inc();
//    if (fibObservedBy == null) return true;
//    final int tid = RVMThread.getCurrentThread().getDrID();
//    final boolean b = fibObservedBy[tid];
//    if (!b) fibObservedBy[tid] = true;
//    else if (Dr.STATS) DrStats.redudantStaticFinalReads.inc();
//    return b;
//  }
  @Uninterruptible
  public final void drSetClassInitVC(WordArray vc) {
    drClassInitVC = vc;
  }
  @Uninterruptible
  @Inline
  public final WordArray drGetClassInitVC() {
    return drClassInitVC;
  }
  // end FIB.
