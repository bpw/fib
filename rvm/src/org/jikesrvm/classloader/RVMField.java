...
public final class RVMField extends RVMMember {
  ...
  /**
   * FIB: metadata offsets
   * For normal fields (FibTreatment.check(this)), this is the offset
   * from the object address (or from zero for statics) of the start of the
   * access history.
   * For volatile fields (FibTreatment.vol(this)), this is the offset
   * from the object address (or from zero for statics) of the start of the
   * access history.
   * 
   * A value with the LSB set is (minus that bit) an offset relative to
   * JTOC that needs lazy absolution.
   * @return
   */
  private Offset drOffset = Offset.fromIntSignExtend(NO_OFFSET);
  ...
  // FIB: metadata offsets
  @Uninterruptible
  public final boolean hasDrOffset() {
    return drOffset.toInt() != NO_OFFSET;
  }
  @UninterruptibleNoWarn("Runtime uninterruptible, compile-time interruptible")
  public final Offset getDrOffset() {
    if (VM.VerifyAssertions)  VM._assert(hasDrOffset(), "FIB Offset request but don't have one.");
    Offset off;
    if (VM.runningVM) {
      off = drOffset;
      if (!off.toWord().and(Word.one()).isZero()) {
        if (!this.isStatic()) VM.sysFail("getting DR offset of field that lacks one");
        final Offset old = off;
        off = Magic.getTocPointer().plus(off.toWord().and(Word.one().not()).toOffset()).toWord().toOffset();
        ObjectReference.fromObject(this).toAddress().attempt(old.toWord(), off.toWord(), Entrypoints.drOffsetField.getOffset());
      }
    } else {
      synchronized (this) {
        off = drOffset;
  ...
  public final void setDrOffset(Offset offset) {
    if (VM.VerifyAssertions) {
      if ((offset.toInt() % Epoch.BYTES_IN_EPOCH) != 0) {
        VM._assert(false, "Misaligned FIB offset: " + this.getName().toString() + "  " + offset.toInt());
      }
    }
    drOffset = offset;
  }
  @UninterruptibleNoWarn
  public final void setDrJTOCOffset(Offset offset) {
    if (VM.VerifyAssertions) {
      if ((offset.toInt() % Epoch.BYTES_IN_EPOCH) != 0) {
        VM._assert(false, "Misaligned FIB offset: " + this.getName().toString() + "  " + offset.toInt());
      }
    }
    drOffset = offset.toWord().or(Word.one()).toOffset();
  }
  ...
  public void setObjectValueUnchecked(Object obj, Object ref) {
    if (isStatic()) {
      ...
      if (Esc.getConfig().escapeInsertBarriers() && Esc.getConfig().escapeInstrumentVM() && VM.runningVM) {
        EscapeState.writeStatic(ObjectReference.fromObject(ref).toAddress().toWord(),
            getOffset(),
            EscInstrDecisions.passFieldInfo() ? (Esc.getConfig().escapeUseFieldOffset() ? getOffset().toInt() : getId()) : 0,
            0);
      }
      ...
      // Escape: Instrumenting VM need to trace all writes.
      if (Esc.getConfig().escapeInsertBarriers() && Esc.getConfig().escapeInstrumentVM()) {
        EscapeState.writeObject(ObjectReference.fromObject(ref).toAddress().toWord(),
            ObjectReference.fromObject(obj).toAddress(),
            getOffset(),
            EscInstrDecisions.passFieldInfo() ? (Esc.getConfig().escapeUseFieldOffset() ? getOffset().toInt() : getId()) : 0,
            0);
      }
