...
public final class Lock implements Constants {
  ...
  @UnpreemptibleNoWarn
  /**
   * Tries to acquire this heavy-weight lock on the indicated object without blocking.
   *
   * @param o the object to be locked
   * @return true, if the lock succeeds; false, otherwise
   */
  @Unpreemptible
  public boolean tryLockHeavy(Object o) {
    // if (tentativeMicrolocking) {
    if (!mutex.tryLock()) {
      return false;
    }
    ...
    //   mutex.lock();  // Note: thread switching is not allowed while mutex is held.
    // }
    return tryLockHeavyLocked(o);
  }
  /** Tries to complete the task of acquiring the heavy lock, assuming that the mutex
      is already acquired (locked). */
  @Unpreemptible
  public boolean tryLockHeavyLocked(Object o) {
    if (lockedObject != o) { // lock disappeared before we got here
      mutex.unlock(); // thread switching benign
      return false;
    }
