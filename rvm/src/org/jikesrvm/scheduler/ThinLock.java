...
public final class ThinLock implements ThinLockConstants {
  ...
  @NoNullCheck
  @Unpreemptible
  public static boolean inlineTryLock(Object o, Offset lockOffset) {
    Word old = Magic.prepareWord(o, lockOffset); // FIXME: bad for PPC?
    Word id = old.and(TL_THREAD_ID_MASK.or(TL_STAT_MASK));
    Word tid = Word.fromIntSignExtend(RVMThread.getCurrentThread().getLockingId());
    if (id.EQ(tid)) {
  ...
  public static void lock(Object o, Offset lockOffset) {
    ...
    for (int cnt=0;;cnt++) {
      ...
      if (stat.EQ(TL_STAT_BIASABLE)) {
        ...
        // Octet: before and after lockHeavy(), enter and leave the blocked state        
  ...
  @NoNullCheck
  @Unpreemptible
  public static boolean tryLock(Object o, Offset lockOffset) {
    if (STATS) fastLocks++;
    ...
    for (int cnt=0;;cnt++) {
      ...
      if (stat.EQ(TL_STAT_BIASABLE)) {
        ...
        if (id.isZero()) {
          ...
          // Might need tryCasFromBiased (give up if handshake required)
          if (casFromBiased(o, lockOffset, old, biasBitsToThinBits(old), cnt)) {
            continue; // don't spin, since it's thin now
          }
        }
        ...
        if (id.isZero()) {
          ...
          // tryToInflate=true;
          return false; // too much contention for tryLock
        }
      } else {
        if (VM.VerifyAssertions) VM._assert(stat.EQ(TL_STAT_FAT));
        
        // lock is fat.  try to lock it without blocking.
        if (Lock.getLock(getLockIndex(old)).tryLockHeavy(o)) {
          return true;
        }
      }
      ...
      if (tryToInflate) {
        ...
        // NB: this calls into our attemptToMarkInflated() method, which will do the
        // Right Thing if the lock is biased to someone else.
        if (tryInflateAndLock(o, lockOffset)) {
          return true;
        }
      } else {
  ...
  public static void unlock(Object o, Offset lockOffset) {
    ...
    for (int cnt=0;;cnt++) {
      ...
      if (stat.EQ(TL_STAT_BIASABLE)) {
        ...
        // Octet: before and after unlockHeavy(), enter and leave the blocked state
        if (Octet.getConfig().doCommunication()) {
          Stats.blockCommLock.inc();
          Communication.blockCommunicationRequests(true);
        }
  ...
  @Uninterruptible
  @NoNullCheck
  public static boolean holdsLockOrBiasUncontended(RVMThread thread, Object o) {
    final Offset lockOffset = ObjectModel.getThinLockOffset(o);
    int tid = thread.getLockingId();
    Word bits = Magic.getWordAtOffset(o, lockOffset);
    if (bits.and(TL_STAT_MASK).EQ(TL_STAT_BIASABLE)) {
      // if locked, then it is locked with a thin lock
...
//DR/FIB
 @Uninterruptible
 @NoNullCheck
 public static boolean heldOrBiased(Object o) {
   final Offset lockOffset = ObjectModel.getThinLockOffset(o);
   int tid = RVMThread.getCurrentThread().getLockingId();
   Word bits = Magic.getWordAtOffset(o, lockOffset);
   if (bits.and(TL_STAT_MASK).EQ(TL_STAT_BIASABLE)) {
     return bits.and(TL_THREAD_ID_MASK).toInt() == tid;
   } else if (bits.and(TL_STAT_MASK).EQ(TL_STAT_THIN)) {
  ...
  @NoInline
  @Unpreemptible
  public static boolean tryCasFromBiased(Object o, Offset lockOffset,
                                      Word oldLockWord, Word changed,
                                      int cnt) {
    RVMThread me=RVMThread.getCurrentThread();
    Word id=oldLockWord.and(TL_THREAD_ID_MASK);
    if (id.isZero()) {
      ...
      if (owner==me /* I own it, so I can unbias it trivially.  This occurs
        ...
        return false;  // give up, otherwise might block.
      }
    }
  }
  ...
  @Unpreemptible
  private static boolean tryInflateAndLock(Object o, Offset lockOffset) {
    Lock l = Lock.allocate();
    if (l == null) return false; // can't allocate locks during GC
    // Might need tryAttemptToInflate (give up if handshake required)
    Lock rtn = attemptToInflate(o, lockOffset, l);
    if (l != rtn) {
      l = rtn;
      l.mutex.lock();
    }
    return l.tryLockHeavyLocked(o);
  }
