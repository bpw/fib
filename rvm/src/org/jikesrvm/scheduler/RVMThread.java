...
public final class RVMThread extends ThreadContext implements Constants {
  ...
  private static volatile int nextOctetThreadID;
  ...
  void releaseThreadSlot() {
   ...
   public RVMThread(byte[] stack, Thread thread, String name, boolean daemon, SystemThread systemThread, int priority) {
     ...
     // Escape: set RVMThread and SystemThread to ESCAPED. It should be before the initialization of any field of this object. 
     // Man: It's not late to set system thread escaped here, since the constructor of SystemThread calls this method first.
     if (VM.runningVM) {
       if (Esc.getConfig().escapeUseOneBitInGCHeader()) {
         EscapeState.setEscaped(this);
         if (systemThread != null) {
           EscapeState.setEscaped(systemThread);
         }
       } else if (Esc.getConfig().escapeInstrumentAllocation()) {
         Word metadata = EscapeState.getEscapeMetadata();
         Address addr = ObjectReference.fromObject(this).toAddress();
         addr.store(metadata, MiscHeader.ESCAPE_OFFSET);
         if (systemThread != null) {
           addr = ObjectReference.fromObject(systemThread).toAddress();
           addr.store(metadata, MiscHeader.ESCAPE_OFFSET);
         }
       }
     }
    ...
    if (!VM.runningVM) {
      ...
      if (VM.VerifyAssertions && Octet.getConfig().doCommunication()) { // FIB: only assert this if doing Octet stuff...
        ...
        VM._assert((addr1.toInt() & ~0x07) == (addr2.toInt() & ~0x7), "Thread-" + threadSlot + ": takeYieldPoint (" + addr1.toInt() + ") and octetRequestsField (" + addr2.toInt() + ") are not in the same cache line."); // make sure high 29 bits match
      }
      
      // Escape: Mark java.lang.Thread object escaped and perform transitive closure computation.
      // It's not late here, since the new thread has not started, and the tracing will mark all its fields escaped. 
      if (Esc.getConfig().escapeInsertBarriers()) {
        EscapeState.writeStatic(ObjectReference.fromObject(this.thread).toAddress().toWord(), null, 0, 0);
      }
    }

    // Octet: allocate ThreadSafeCounters for this threadSlot.
    Stats.ThreadSafeCounter.allocateForThreadSlot(threadSlot);
    ...
    // FIB:
    drFibComm = (Dr.ON && this.isOctetThread()) ? DrRuntime.newThread(this) : null;
    drThreadVC = drFibComm != null ? drFibComm.vc : null;
    if (Dr.ON && drThreadVC != null) {
      DrDebug.lock();
      DrDebug.twrite(); VM.sysWrite(" started new thread "); DrDebug.twrite(this); VM.sysWriteln();
      DrDebug.unlock();
    }
  ...
  int block(BlockAdapter ba, boolean asynchronous) {
    ...
    if (Dr.COMMUNICATION) DrRuntime.block();
    ...
    if (Dr.COMMUNICATION) DrRuntime.unblock(true);
  ...
  public void endPairHandshake() {
    ...
    if (Dr.COMMUNICATION) DrRuntime.unblock(false);
  }
  ...
  private void handleDebugRequestForThread() {
    ...
    if (!isAboutToTerminate) {
      ...
      if (isInJava()) {
        ...
        // Octet: Force printing of stack trace if needed.        
        long startTime = Time.currentTimeMillis();
        while (asyncDebugRequestedForThisThread) {
          if (Time.currentTimeMillis() - startTime > 100) {
            Stats.tryToPrintStack(this.framePointer);
            break;
          }
          Magic.readCeiling(); // so the read of asyncDebugRequestedForThisThread doesn't get hoisted out of the loop
        }
  ...
  public void start() {
    // FIB:
    if (Dr.THREADS && getCurrentThread().isDrThread() && this.isDrThread()) {
      DrRuntime.fork(this);
    }
    // end FIB
  ...
  private void terminateUnpreemptible() {
    ...
    if (Dr.ON && isDrThread()) {
      DrRuntime.terminateThread(this);
    }
  ...
  public void suspend() {
    ...
    if (Dr.SYNC) {
      DrRuntime.genericUnlock(thread);
    } else {
  ...
  /**
   * FIB: added an extra argument, isApplication, which determines whether FIB
   * instrumentation is applied.
   * 
   * @param o
   * @param hasTimeout
   * @param whenWakeupNanos
   * @param isApplication
   */
  @Interruptible
  void waitImpl(Object o, boolean hasTimeout, long whenWakeupNanos, final boolean isApplication) {
    boolean throwInterrupt = false;
    Throwable throwThis = null;
    if (asyncThrowable != null) {
      ...
      // FIB:
      if (Dr.COMMUNICATION) DrRuntime.block();
      final boolean isDR = Dr.LOCKS && isApplication; 
      if (isDR) {
        DrRuntime.prewait(this, o);
      }
  ...
  public static void wait(Object o, boolean isApplication) throws InterruptedException {
    // FIB: extra argument: do instrument
    getCurrentThread().waitImpl(o, false, 0, isApplication);
  }
//  @Interruptible
//  public static void waitNoFib(Object o) throws InterruptedException {
//    // FIB: extra argument: do NOT instrument
//    getCurrentThread().waitImpl(o, false, 0, false);
//  }
   ...
   */
  @Interruptible
  public static void wait(Object o, long millis, boolean isApplication) {
    long currentNanos = sysCall.sysNanoTime();
    // FIB: extra argument: do instrument
    getCurrentThread().waitImpl(o, true, currentNanos + millis * 1000 * 1000, isApplication);
  }
   ...
   */
  @Interruptible
  public static void waitAbsoluteNanos(Object o, long whenNanos, boolean isApplication) {
    // FIB: extra argument: do instrument
    getCurrentThread().waitImpl(o, true, whenNanos, isApplication);
  }
  ...
  public void join(long ms, int ns) throws InterruptedException {
    if (VM.VerifyAssertions)
      VM._assert(RVMThread.getCurrentThread() != this);
    if (traceBlock)
    ...
    synchronized (this) {
      if (ms == 0 && ns == 0) {
        while (!isJoinable) {
          wait(this, false); // DR adds 2nd arg: Not application context.
          if (traceBlock)
        ...
        while (!isJoinable) {
          waitAbsoluteNanos(this, whenWakeup, false); // DR adds 3rd arg: Not application context.
      ...
      // FIB:
      if (Dr.THREADS && RVMThread.getCurrentThread().isDrThread() && this.isDrThread() && this.isJoinable) {
        DrRuntime.join(this);
      }
      // end FIB
    }
  ...
  // FIB: ===========================================================
  /**
   * Vector clock representing this thread's synchronization frontier.
   */
  public final WordArray drThreadVC;
  /**
   * Is this a thread of interest?
   * @return
   */
  @Inline
  public boolean isDrThread() {
    return VM.runningVM && drEpoch.NE(Epoch.NONE); // isOctetThread();
  }
  /**
   * Get the Fib ID of this thread.
   * @return
   */
  @Inline
  public int getDrID() {
    return Epoch.tid(drEpoch);
  }
  /**
   * This thread's current epoch.
   */
  private Word drEpoch;
  /**
   * Increment the current epoch.
   */
  public Word incDrEpoch() {
    final Word now = drEpoch;
    if (Epoch.tid(now) != 0 || 1 < DrRuntime.maxLiveDrThreads()) {
      final Word e = Epoch.inc(now);
      drEpoch = e;
      if (Dr.STATS) this.drFibComm.reportNewEpoch();
      return e;
    } else {
      return now;
    }
  ...
  /**
   * Set the current epoch to e.
   * @param e
   */
  public void setDrEpoch(Word e) {
    drEpoch = e;
  }
  /**
   ...
   */
  @Inline
  public Word getDrEpoch() {
    return drEpoch;
  }

  private long drRaces = 0;
  @Inline
  public long incDrRaces() {
    return ++drRaces;
  }
  public long getDrRaces() {
    return drRaces;
  }
  
  // (non-)reentrancy checking.
  /**
   * Is this thread already executing within Fib analysis context?
   */
  private int inDR = 0;
  final private int[] drContextStack = new int[4];
  final private ObjectReferenceArray drContextStackTarget = ObjectReferenceArray.create(4);
  final private OffsetArray drContextStackOffset = OffsetArray.create(4);
  private boolean drBecameReentrant = false;
  
  /**
   * There is something odd about a getfieldResolved in java/util/Properties object.
   * Filtering that out since it seems to cause spurious warnings about reentrancy.
   * I suspect that some getfieldResolved is actually triggering an exception or other
   * non-standard return where we are not properly catching the stack unwind.  The same
   * issue appears to arise when tracking lock stacks, but Swarnendu, Man, and Mike
   * have warned that this is tricky.
   */
  private static final Atom PROPERTIES_DESC = Atom.findOrCreateAsciiAtom("Ljava/util/Properties;");
  /**
   * Enter Fib analysis context.
   */
  @NoInline
  @NoCheckStore
  private boolean enterDR(Object target, Offset offset, boolean assertDrThread) {
    if (VM.VerifyAssertions) {
      if (assertDrThread) VM._assert(isDrThread());

      if (Dr.COMMUNICATION) FibComm.assertValidRefs();
//      if (ep.equals(Entrypoints.drGetFieldResolvedMethod) && target != null
//          && ObjectModel.getObjectType(target).getDescriptor().equals(PROPERTIES_DESC)) {
//        return true;
//      }
      // 2nd caller: ____ -> enterDr(wrapper) -> enterDR(this one)
      drContextStack[inDR] = Magic.getCompiledMethodID(Magic.getCallerFramePointer(Magic.getCallerFramePointer(Magic.getFramePointer())));
      drContextStackTarget.set(inDR, ObjectReference.fromObject(target));
      drContextStackOffset.set(inDR, offset);
      if (inDR > 0) {
        DrDebug.lock();
        DrDebug.twriteln("WARNING: reentering DR.");
        printDrContextStack();
        RVMThread.dumpStack();
        DrDebug.unlock();
        drBecameReentrant = true;
        VM.sysFail("reentering DR");
      }
      ++inDR;
      return !drBecameReentrant;
    }
    return true;
  }
  @NoInline
  public boolean enterDR(Object target, Offset offset) {
    return enterDR(target, offset, true);
  }
  @NoInline
  public boolean enterDR() {
    return enterDR(null, Offset.zero(), true);
  }
  /**
   * Leave Fib analysis context.
   */
  @NoInline
  @NoCheckStore
  private void exitDR(Object target, Offset offset, boolean assertDrThread) {
    if (VM.VerifyAssertions) {
      if (assertDrThread) VM._assert(isDrThread());
      
      if (Dr.COMMUNICATION) FibComm.assertValidRefs();
//      VM._assert(inFIB, "BAD: exit Fib when not in Fib.");
//      inFIB = false;
//      if (ep.equals(Entrypoints.drGetFieldResolvedMethod) && target != null
//          && ObjectModel.getObjectType(target).getDescriptor().equals(PROPERTIES_DESC)) {
//        return;
//      }
      --inDR;
      // 2nd caller: ____ -> exitDr(wrapper) -> exitDR(this one)
      final int ep = Magic.getCompiledMethodID(Magic.getCallerFramePointer(Magic.getCallerFramePointer(Magic.getFramePointer())));

      if (inDR > 0) {
//        DrDebug.lock();
//        DrDebug.twriteln("WARNING: exiting FIB, still reentrant.");
//        VM.sysWriteln("exiting from ", ep.getName());
//        printDrContextStack();
//        RVMThread.dumpStack();
//
//        DrDebug.unlock();
      } else if (inDR == 0 && drBecameReentrant) {
//        DrDebug.lock();
//        DrDebug.twrite();
//        VM.sysWriteln("WARNING: exiting last FIB context ", ep.getName());
//        DrDebug.unlock();
        drBecameReentrant = false;
      } else if (inDR < 0){
        DrDebug.lock();
        DrDebug.twrite();
        VM.sysWriteln("WARNING: negative FIB context depth intending to exit",
            CompiledMethods.getCompiledMethod(ep).getMethod().getName());
        DrDebug.unlock();
        VM.sysFail("oops-");
      }
      
      if (inDR >= 0 && drContextStack[inDR] != ep) {
        DrDebug.lock();
        DrDebug.twrite();
        VM.sysWrite("WARNING: mismatched FIB context exit, from ");
        VM.sysWrite(CompiledMethods.getCompiledMethod(ep).getMethod().getName());
        VM.sysWriteln(" expecting ", CompiledMethods.getCompiledMethod(drContextStack[inDR]).getMethod().getName());
        DrDebug.unlock();
        VM.sysFail("oops*");
      }
      // drContextStack[inDR] = 0;
      drContextStackTarget.set(inDR, ObjectReference.nullReference());
      drContextStackOffset.set(inDR, Offset.zero());
 
      // Debug.twriteln("exitFib");
    }
  }
  @NoInline
  public void exitDR() {
    exitDR(null, Offset.zero(), true);
  }
  @NoInline
  public void exitDR(Object target, Offset offset) {
    exitDR(target, offset, true);
  }
  
  @NoCheckStore
  public void exitDrIfMatch(int ep) {
    if (VM.VerifyAssertions && Dr.ON) {
      if (inDR > 0 && drContextStack[inDR-1] == ep) {
        --inDR;
        drContextStackTarget.set(inDR, ObjectReference.nullReference());
        drContextStackOffset.set(inDR, Offset.zero());
        DrDebug.lock();
        DrDebug.twrite();
        RVMMethod m = CompiledMethods.getCompiledMethod(ep).getMethod();
        VM.sysWriteln("WARNING: unwinding stack, exited DR frame: ",
            m.getName());
        DrDebug.unlock();
        VM.sysFail("May be raising exception in DR code.");
      }
    }
  }

  
  public void printDrContextStack() {
    for (int i = inDR; i >= 0; i--) {
      VM.sysWrite("  [", i, "] in ");
      VM.sysWrite(CompiledMethods.getCompiledMethod(drContextStack[i]).getMethod().getName());
      if (!drContextStackTarget.get(i).isNull()) {
        VM.sysWrite(" target = ", ObjectReference.fromObject(drContextStackTarget.get(i)).toAddress());
        VM.sysWrite(" offset = ", drContextStackOffset.get(i));
        VM.sysWriteln(" target type ", ObjectModel.getTIB(drContextStackTarget.get(i)).getType().getTypeRef().getName());
      } else if (!drContextStackOffset.get(i).isZero()) {
        VM.sysWrite(" target = static");
        VM.sysWrite(" offset = ", drContextStackOffset.get(i));
      }
      VM.sysWriteln();
    }
  ...
  public boolean isInDR() {
    return inDR > 0;
  }
  
  public final FibComm drFibComm;
  
  public WordArray drActiveVolatileVC = null;
  
  public int drBufferNext = 0;
  public int drBufferGCgen = 0;
  
  public static RVMThread threadByLockingId(int lid) {
    return threadBySlot[lid >> ThinLockConstants.TL_THREAD_ID_SHIFT];
  }
  
  public WordArray drEpochMapStash;
  
  public long drGcCount = 0, drGcCountStash = 0;
  
  public final BitVector drClassInitsObserved = new BitVector(16384);
  // end FIB
