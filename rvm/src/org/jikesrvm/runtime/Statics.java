...
public class Statics implements Constants {
  ...
  // FIB: support for non-reference items in the reference section.
  // We use this to position volatiles adjacent to VC pointers, etc.
  
//  /**
//   * Number of non-reference slots in the reference section.
//   */
//  private static int numBogusReferenceSlots = 0;
//  /**
//   * indexes of non-reference and maybe-reference slots in the reference slot section.
//   */
//  private static int[] bogusReferenceSlots = new int[256];
  ...
  public static synchronized Offset allocateNumericSlot(int size, boolean field) {
    ...
    // DR
    if (size == 4*BYTES_IN_INT) {
      // nextNumberSlot refers to next available slot.
      // add 3 to make a chunk of 4 available.
      slot = nextNumericSlot - 3;
      // Align down to multiple of 4 slots, saving a hole if any.
      if ((slot & 3) != 0) {
        numericSlotHole = nextNumericSlot;
      }
      slot -= (slot & 3);
      // Save next available slot.
      nextNumericSlot = slot - 1;
      // Mark fields as non-literals.
      if (field) {
      ...
      if (VM.VerifyAssertions) VM._assert(size <= 4);
  ...
  // DR/FIB: Access histories of static fields.

  /**
   * Number of static access histories allocated.
   */
  private static int drAccessHistoryCount = 0;
  /**
   * Get number of static access histories allocated.
   * @return
   */
  @Uninterruptible
  public static int getDrAccessHistoryCount() {
    return drAccessHistoryCount;
  }
  /**
   * JTOC Offsets of static access histories.
   */
  private static OffsetArray drAccessHistoryOffsets = OffsetArray.create(256);
  /**
   * Get JTOC Offsets of static access histories.
   */
  @Uninterruptible
  public static OffsetArray getDrAccessHistoryOffsets() {
    return drAccessHistoryOffsets;
  }
  /**
   * Allocate a static access history.  Allocations are in the numeric section.
   * They are scanned separately during GC.
   * @return
   */
  public static synchronized Offset allocateDrAccessHistory() {
    if (VM.VerifyAssertions) {
      VM._assert(Dr.CHECK_STATICS);
    }
    final Offset jtocOffset = allocateNumericSlot(AccessHistory.BYTES_IN_HISTORY, true);
    // Expand if necessary.
    if (drAccessHistoryCount == drAccessHistoryOffsets.length()) {
      final OffsetArray offs = OffsetArray.create(2*drAccessHistoryOffsets.length());
      for (int i = 0; i < drAccessHistoryOffsets.length(); i++) {
        offs.set(i, drAccessHistoryOffsets.get(i));
      }
      drAccessHistoryOffsets = offs;
    }
    // Insert new offset.
    drAccessHistoryOffsets.set(drAccessHistoryCount++, jtocOffset);
    return jtocOffset;
  }
  ...
  public static void setSlotContents(Offset offset, Object object) {
    ...
    // Escape: Instrumenting VM need to trace all writes.
    if (Esc.getConfig().escapeInsertBarriers() && Esc.getConfig().escapeInstrumentVM() && VM.runningVM) {
      EscapeState.writeStatic(ObjectReference.fromObject(object).toAddress().toWord(),
          offset,
          0,
          0);
    }
