...
public class RuntimeEntrypoints implements Constants, ArchitectureSpecific.StackframeLayoutConstants {
  ...
  public static void athrow(Throwable exceptionObject) {
    if (VM.VerifyAssertions && RVMThread.getCurrentThread().isInDR()) {
      DrDebug.lock();
      DrDebug.twriteln("WARNING: athrow while in Dr.");
      // RVMThread.getCurrentThread().printDrContextStack();
      RVMThread.dumpStack();
      DrDebug.unlock();
    }
  ...
  static void unlockAndThrow(Object objToUnlock, Throwable objToThrow) {
    if (VM.VerifyAssertions && RVMThread.getCurrentThread().isInDR()) {
      DrDebug.lock();
      DrDebug.twriteln("WARNING: unlockAndThrow while in Dr.");
      // RVMThread.getCurrentThread().printDrContextStack();
      RVMThread.dumpStack();
      DrDebug.unlock();
    }
  ...
  @NoInline
  @Entrypoint
  static void drUnlockAndThrow(Object objToUnlock, Throwable objToThrow) {
    if (VM.VerifyAssertions && RVMThread.getCurrentThread().isInDR()) {
      DrDebug.lock();
      DrDebug.twriteln("WARNING: drUnlockAndThrow while in Dr.");
      // RVMThread.getCurrentThread().printDrContextStack();
      RVMThread.dumpStack();
      DrDebug.unlock();
    }
    DrRuntime.genericUnlock(objToUnlock);
    athrow(objToThrow);
  }
    ...
    while (Magic.getCallerFramePointer(fp).NE(STACKFRAME_SENTINEL_FP)) {
      ...
      if (compiledMethodId != INVISIBLE_METHOD_ID) {
        ...
        if (VM.VerifyAssertions && Dr.ON) {
          RVMThread.getCurrentThread().exitDrIfMatch(compiledMethodId);
        }
