...
public final class MemoryManager implements HeapLayoutConstants, Constants {
  ...
  private static int pickAllocatorForType(RVMType type) {
    ...
    if (type.isArrayType()) {
      ...
      if (elementType.isPrimitiveType() || elementType.isUnboxedType()){
        // FIB: seems this is treated same as ALLOC_DEFAULT, so probably not a problem for using
        // non-ref type arrays and adding custom scanning/tracing.
    ...
    // Escape: Also make it in the immortal space.
    if ((Octet.getConfig().instrumentAllocation() 
        || Esc.getConfig().escapeUseOneBitInGCHeader()
        || Esc.getConfig().escapeInstrumentAllocation() ) && isPrefix("Lorg/jikesrvm/scheduler/RVMThread;", typeBA)) {
  ...
  public static void initializeHeader(BootImageInterface bootImage, Address ref, TIB tib, int size,
    ...
    // Escape: Mark objects in boot image escaped.
    if (Esc.getConfig().escapeUseOneBitInGCHeader()) {
      status = (byte)EscapeState.getEscapedState(Word.fromIntZeroExtend(status)).toInt();
    }
