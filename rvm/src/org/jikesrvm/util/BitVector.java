...
public final class BitVector implements Serializable {
  ...
  /**
   * DR: Set all bits with indices <= this bit.
   */
  public void setUpThrough(int bit) {
    // Set all in preceding buckets.
    for (int i = 0; i < subscript(bit); i++) {
      bits[i] = -1;
    }
    // Set prefix of this bucket.
    int shiftBits = bit & LOW_MASK;
    int mask = (1 << shiftBits);
    bits[subscript(bit)] = mask | (mask - 1);
  }
